...
DEG.markers_triple <- FindMarkers(m26_myc_triple, ident.1 = "M+2+6-", ident.2 = "triple positive", slot = "counts")

library(EnhancedVolcano)

EnhancedVolcano(DEG.markers_triple , 
                rownames(DEG.markers_triple ),
                x ="avg_log2FC", 
                y ="p_val_adj", FCcutoff = 0.25)
...
markers <- FindMarkers(m26_myc_triple, ident.1 = "M+2+6-", ident.2 = "triple positive", test.use="MAST", logfc.threshold = 0, slot = "counts")
...
S3<- ggplot(fgseaRes, aes(x= NES, y=reorder(pathway,NES), size=size, color=pval, group=NES)) + geom_point(alpha = 0.8) + labs(x="Normalized Enrichment Score", y="Pathways",title="Hallmark pathways NES from GSEA") + theme_minimal()
S3
